...
TEST_CASE("getSerializedJSONAssignment - Consistency with getJSONAssignment", "[serialized-json]") {
    // Load flags configuration
    std::string flagsPath = "test/data/ufc/flags-v1.json";
    ConfigResponse configResponse = loadFlagsConfiguration(flagsPath);
    ...
    SECTION("Serialized result matches getJSONAssignment dump") {
        Attributes aliceAttrs;
        aliceAttrs["email"] = std::string("alice@mycompany.com");
        aliceAttrs["country"] = std::string("US");

        json jsonDefaultValue = json::parse(R"({"foo":"bar"})");
        std::string stringDefaultValue = jsonDefaultValue.dump();

        // Get assignment using both methods
        json jsonResult = client->getJSONAssignment(
            "json-config-flag",
            "alice",
            aliceAttrs,
            jsonDefaultValue
        ...
        std::string serializedResult = client->getSerializedJSONAssignment(
            "json-config-flag",
            "alice",
            aliceAttrs,
            stringDefaultValue
        ...
        // The serialized result should match the dump of the JSON result
        CHECK(serializedResult == jsonResult.dump());

        // When parsed, they should be equal
        json parsedSerializedResult = json::parse(serializedResult);
        CHECK(parsedSerializedResult == jsonResult);
    }

    SECTION("Both methods return defaults consistently") {
        Attributes attrs;

        json jsonDefaultValue = json::parse(R"({"default":"value"})");
        std::string stringDefaultValue = jsonDefaultValue.dump();

        // Get assignment for non-existent flag using both methods
        json jsonResult = client->getJSONAssignment(
            "non-existent-flag",
            "test-subject",
            attrs,
        ...
        CHECK(jsonResult == jsonDefaultValue);
        CHECK(serializedResult == stringDefaultValue);
        CHECK(serializedResult == jsonResult.dump());
    }
}
